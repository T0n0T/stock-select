...
- `analyze-symbol`
  ...
  - 按 `b1` / `b2` / `dribull` / `hcr` 的现有实现复用对应的单股确定性分析与 baseline review
...
- 入选后会额外写出 `yellow_b1` 标注，识别主图公式中的黄色 `B1` 信号：`B1_环境 AND 转色 AND J < 29 AND PCT <= 3.7`，并要求近 5 日原始黄 B1 次数不超过 3 次
