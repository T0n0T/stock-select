...
def load_prepared(method: str, end_date: str) -> dict[str, pd.DataFrame]:
    normalized_method = method.strip().lower()
    if normalized_method in SHARED_PREPARED_METHODS:
        pattern = "*.pkl"
        suffix = ".pkl"
    else:
        pattern = f"*.{normalized_method}.pkl"
        suffix = f".{normalized_method}.pkl"

    candidates = []
    for path in sorted(PREPARED_DIR.glob(pattern)):
        name = path.name
        if normalized_method in SHARED_PREPARED_METHODS and any(
            name.endswith(f".{other}.pkl") for other in {"hcr"}
        ):
            continue
        if not name.endswith(suffix):
            continue
        date_part = name.removesuffix(suffix)
        if date_part <= end_date:
            candidates.append(path)

    if not candidates:
        raise FileNotFoundError(
            f"No prepared pickle found for method={method} on or before {end_date} in {PREPARED_DIR}"
        )

    payload = pickle.loads(candidates[-1].read_bytes())
    return payload.get("prepared_by_symbol", payload)
