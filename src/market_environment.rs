    ...
    F: FnOnce() -> anyhow::Result<EnvironmentEvaluation>,
{
    if let Some(state) = manual_state {
        let record = EnvironmentRecord {
            pick_date,
            state: normalize_environment_state(&state)?,
            score_based_state: normalize_environment_state(&state)?,
            rule_based_state: normalize_environment_state(&state)?,
            vote_based_state: normalize_environment_state(&state)?,
...
pub fn resolve_market_environment(
    ...
    if records.is_empty() {
        anyhow::bail!("No market environment interval covers pick_date {pick_date}.");
    }
...
pub fn evaluate_market_environment(
    ...
    let score_based_state = if total_score >= 10.0 {
        "strong"
    ...
    let vote_based_state = vote_state(&[sse.state_hint.as_str(), cn2000.state_hint.as_str()]);
    let rule_based_state = combine_rule_state(sse.state_hint.as_str(), cn2000.state_hint.as_str());
...
fn load_environment_records(runtime_root: &Path) -> anyhow::Result<Vec<EnvironmentRecord>> {
        ...
        .lines()
    {
        ...
        records.push(serde_json::from_str::<EnvironmentRecord>(line)?);
    }
    Ok(records)
}
...
fn write_environment_records(
    ...
    for record in records {
        let path = daily_dir(runtime_root).join(format!(
            "{}.{}.json",
    ...
    let intervals = build_intervals(records)
        ...
        .map(|interval| {
            json!({
                ...
                "evaluated_at": interval.evaluated_at,
                "source": interval.source,
                "manual_override": interval.manual_override,
...
fn vote_state(states: &[&str]) -> String {
