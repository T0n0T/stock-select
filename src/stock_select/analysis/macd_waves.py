...
def classify_weekly_macd_wave(frame: pd.DataFrame, pick_date: str) -> MacdWaveClassification:
    ...
    bullish = bool(dif.iloc[-1] > dea.iloc[-1] and latest_hist >= previous_hist)
    had_pullback = bool((hist < 0).any())
    if not had_pullback and len(weekly_close) >= 2:
        had_pullback = bool(float(weekly_close.iloc[-1]) < float(weekly_close.max()))
...
def classify_daily_macd_wave(frame: pd.DataFrame, pick_date: str) -> MacdWaveClassification:
    ...
    bullish_now = bool(dif.iloc[-1] > dea.iloc[-1] and hist.iloc[-1] >= hist.iloc[-2])
    ...
    if shrinking_positive and converging and third_wave_gain > 0.30:
        return MacdWaveClassification(
            "invalid",
            False,
    ...
    if shrinking_positive and converging:
        return MacdWaveClassification(
            "wave2_end",
            True,
