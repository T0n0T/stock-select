...
#[test]
fn review_list_accepts_positive_limit() {
    let mut cmd = Command::cargo_bin("stock-select-rs").unwrap();
    cmd.args(["review-list", "--limit", "1"])
        .assert()
        .success()
        .stdout(predicate::str::contains("review-list"));
}
...
fn review_list_prefers_selection_display_artifact() {
    ...
    .assert()
    .success()
    .stdout("1\t000002.SZ\t测试二\t-\t0.700000\tKEEP\t-\n");
}
