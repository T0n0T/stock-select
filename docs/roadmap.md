...
28. LightGBM 维护脚本已迁入 `scripts/ml/`：`backfill_candidates.py` 可按数据库交易日并发补齐指定 method 的历史 EOD candidates，当前默认 `--method b2`；`build_rank_dataset.py` 默认从当前 `candidates/<date>.<method>.json` 读取样本集合，训练因子统一装载 Rust 生成的 `<runtime>/factors/<artifact_key>.<method>/factors.json`，数据库行情只用于补 ret label；缺少 factor artifact 时停止并提示先执行 `screen --export-factors`；`--source select` 可回放线上 run/display 样本上下文，但因子来源仍保持一致。训练/score diagnostics 默认写 `diagnostics/ml/<method>/`，发布脚本从当前 `.env` 的 `STOCK_SELECT_RUNTIME_ROOT` 解析 runtime，并发布到 `<runtime>/models/<method>/`；Rust 默认模型解析已移除旧长目录回退，只认当前主目录或显式 override。
...
当前状态：已完成 embedded history raw factor 公式的首轮迁移，并接入 prepared cache history 注入。新增 `src/factors/` 作为 Rust 因子库目录，按因子族拆分为 `ma.rs`、`zx.rs`、`price_position.rs`、`volume.rs`、`macd.rs`、`abnormal_volume.rs` 和 `registry.rs`，`engine` 只负责 payload 适配、排序和 artifact 编排。`screen --method b2 --export-factors` 会在 select/run 前写 `<runtime>/factors/<artifact_key>.b2/factors.json` 与 `manifest.json`；`run --method b2` 也会把实际用于排序的 factors 同步写入同一 runtime factors 目录，同时保留 `select/<key>.b2/factors.json` 作为兼容副本。候选 payload 或 prepared cache 注入的 `history` 现在按 Rust 因子库只计算 raw-only 连续数值因子：价线距离、量能/换手 ratio、120 日箱体位置/宽度/距离、20 日最高收盘距离、MACD 比例/变化/3 日斜率、range compression、abnormal volume event 和 `days_ago` 一类数值量；不再导出 `signal_type`、MACD phase/wave/stage、`*_state` / `*_type` / `*_bucket` 或 `*_flag` 这类状态语义列。`turnover_to_ma5_ratio` 已改回旧训练口径，使用 `daily_market.turnover_rate` 计算最新日换手率相对 5 日均值的比例；`turnover_n` 继续保留在 prepared cache/candidate artifact 中，仅用于 turnover-top pool 和兼容候选展示，不再参与该训练因子。对应训练 dataset schema 也已同步收口，默认训练只保留 `env` 与显式 raw numeric factor 列，不再让 `_flag` 或语义 categorical 进入新 CLI 默认特征集。
...
当前状态：已完成。`scripts/ml/backfill_candidates.py` 可从 `daily_market` 查询交易日，并发执行 `screen --method <method> --pick-date <date>` 补齐历史 EOD candidates，当前默认 b2，默认跳过已有 candidate，支持 dry-run 和失败汇总。`scripts/ml/build_rank_dataset.py` 默认读取当前 `candidates/<date>.<method>.json` 作为样本集合，统一从 `<runtime>/factors/<artifact_key>.<method>/factors.json` 装载 Rust 侧已计算好的训练因子，并用数据库行情补前向收益 label 后输出 `diagnostics/ml/<method>/rank_dataset.csv`；缺少 factor artifact 时会停止并提示先运行 `stock-select-rs screen --method <method> --pick-date <date> --export-factors`，不再用 Python 重复实现 MA、ZX、MACD、异常放量或 env 因子。显式 `--source select` 时读取 `select/<date>.<method>/` 的 `run.json`、`display.json` 作为回放上下文，因子仍来自 runtime factors 目录。`train_rank_lgbm.py` 支持 `--method`、feature set、rolling validation、report 输出和 metadata 构建，并在主训练路径直接写 `feature_manifest.json`、`model.txt` 和 `model_metadata.json`，rolling 调参目录可直接作为候选模型目录进入后续 dry-run / promote。`model-maintenance` skill 已补充受限自迭代调参约定：训练/重训默认最多比较 12 组小网格 trial，按 rolling 指标选择候选，只做 export 和 promote dry-run，不自动发布；训练结束必须汇报 dataset 覆盖、每个 trial 的 rolling 指标、最佳 trial、top features、dry-run 状态、发布建议和剩余风险。`export_lgbm_scores.py` 输出 score CSV、summary，并可为指定 score window 补充独立打分模型产物。`promote_lgbm_model.py` 支持 metadata/report 校验、dry-run、发布、归档、回滚和 `model_card.json`，`scripts/model_maintenance.sh` 则进一步把 `status`、`archives`、`promote`、`dry-run-promote` 和 `switch` 这些常用维护动作统一收口成一个 shell 入口，方便用户查看当前模型、浏览历史归档并切换旧版本。默认从当前 `.env` 读取 `STOCK_SELECT_RUNTIME_ROOT` 并发布到 `<runtime>/models/<method>/`。新增 `model-maintenance` skill 和 references 记录 operator 流程；当前章节已验证 b2，后续可通过 screen 和因子计算层继续扩展其他方法。

剩余边界：真实数据库 dataset 构建、完整训练和发布仍需 operator 在有 DSN 和数据的环境中执行；score CSV、rolling diagnostics 只作为研究材料，不参与 `stock-select-rs run/review` 排序。
